...
const AdminPinModal: React.FC<AdminPinModalProps> = ({ isOpen, onClose, onSuccess }) => {
    ...
    const pinDots = Array(ADMIN_PIN.length).fill(0).map((_, i) => (
        <div key={i} className={`w-4 h-4 rounded-full border-2 transition-all ${i < pin.length ? 'bg-purple-400 border-purple-400 animate-pulse' : 'border-slate-600'}`}></div>
    ));
    ...
    return (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex justify-center items-center z-[99] animate-fade-in p-4" onClick={onClose}>
            <div className={`glass-pane rounded-2xl p-8 w-full max-w-xs text-center animate-slide-in-up ${isWrong ? 'animate-shake' : ''}`} onClick={e => e.stopPropagation()}>
